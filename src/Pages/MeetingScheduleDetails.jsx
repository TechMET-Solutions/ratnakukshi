...
const getStatusToneClass = (status) => {
  ...
  if (
    normalizedStatus === "committee member" ||
...
function MeetingScheduleDetails() {
  ...
  const handleUpdatePresented = async (caseId, presented) => {
    try {
      ...
      const response = await axios.put(
        `${API}/api/update-presented-status`,
        payload,
      );

      console.log("API Response:", response.data);

      alert("Presented status updated successfully");
...
const handleRemoveCase = async (meetingId, caseId) => {
  try {
    const response = await axios.put(
      `${API}/api/remove-case-from-meeting`,
      {
                  ...
                  <tbody>
                    {caseDetails.length > 0 ? (
                      caseDetails.map((item, index) => {
                        const hasQuery =
                          getFeedbackHistory(item).length > 0 ||
                          Boolean(getQueryText(item));
                        ...
                        return (
                          ...
                          >
                            ...
                            <td
                              className={`px-6 py-4 text-sm font-medium ${getStatusToneClass(item.status)}`}
                            >
  ...
  <div className="flex items-center gap-3 justify-center">
    <select
      value={item.presented}
      onChange={(e) =>
        handleUpdatePresented(
          item.id,
                        ...
                        {feedbackHistory.map((item, index) => (
                          ...
                          >
                            <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                              <span
                                className={`rounded-md px-2.5 py-0.5 text-[11px] font-bold uppercase ${
